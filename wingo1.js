...
; // Bisa lebih dari satu grup
let lastSentPeriod = null; // Menyimpan periode terakhir yang dikirim
let predictionHistory = []; // Menyimpan riwayat prediksi (maksimal 20)
let sentResults = new Set(); // Menyimpan periode yang sudah dikirim agar tidak spam

// Fungsi mengirim pesan ke banyak grup Telegram
    ...
    TELEGRAM_CHAT_IDS.forEach(chatId => {
        $.ajax({
            ...
            success: function(response) {
                console.log(`Pesan dikirim ke Telegram [${chatId}]:`, response);
            },
            error: function(error) {
                console.error(`Gagal mengirim pesan ke Telegram [${chatId}]:`, error);
            }
        });
    });
}
...
function fetchMyEmerdList() {
    $.ajax({
        ...
        dataType: "json",
        success: function(response) {
            if (!response.data?.gameslist?.length) {
                console.error("Data MyEmerdList kosong atau tidak ditemukan.");
                return;
            }

            let firstGame = response.data.gameslist[0]; // Ambil game pertama
            showListOrder2(response.data.gameslist, 1);
            fetchNoaverageEmerdList(firstGame);
        },
        error: function(error) {
            console.error("Gagal mengambil MyEmerdList:", error);
        }
    });
}

// Fetch NoaverageEmerdList
function fetchNoaverageEmerdList(firstGame) {
    $.ajax({
        type: "POST",
        ...
        success: function(response) {
            if (!response.data?.gameslist?.length) {
                console.error("Data NoaverageEmerdList kosong atau tidak ditemukan.");
                return;
            }
            ...
            if (firstGame?.stage === list_orders[0]?.period) {
                showGameResult(firstGame);
            }

            processGameData(list_orders);
            checkWinLoss(list_orders);
            showListOrder(list_orders, 0);
            showListOrder_t(list_orders, 2);
        },
        error: function(error) {
            console.error("Gagal mengambil NoaverageEmerdList:", error);
        }
    });
}

// Fungsi menampilkan hasil game di modal
function showGameResult(firstGame) {
    ...
    if (firstGame.get == 0) {
        myModalheader.innerHTML = "Kalah";
        myModal_result.innerHTML = "Rp " + firstGame.money;
    } else {
        myModalheader.innerHTML = "Menang";
        myModal_result.innerHTML = "Rp " + firstGame.get;
    }
    myModal_result_Period.innerHTML = "Periode:" + firstGame.stage;
    ...
    lottery_result.innerHTML = `Lottery Result:
        <span class='btn-boox'>${color}</span>
        <span class='btn-boox'>${firstGame.result}</span>
        <span class='btn-boox'>${type}</span>`;
}

// Fungsi membuat prediksi untuk periode yang akan datang
function processGameData(games) {
    let latestGame = games[0]; // Ambil game terbaru
    let nextPeriod = String(parseInt(latestGame.period) + 1); // Prediksi untuk periode berikutnya

    if (lastSentPeriod === nextPeriod) return; // Jika sudah diproses, lewati

    let randomPrediction = Math.random() < 0.5 ? "Kecil" : "Besar"; // Prediksi acak

    // Simpan prediksi baru
    predictionHistory.push({ period: nextPeriod, prediction: randomPrediction, result: "Menunggu..." });

    // Hapus prediksi lama jika sudah lebih dari 20
    if (predictionHistory.length > 20) {
        predictionHistory.shift();
    }

    // Kirim riwayat prediksi ke Telegram
    sendPredictionHistory();

    lastSentPeriod = nextPeriod;
}

// Fungsi mengecek hasil menang/kalah
function checkWinLoss(games) {
    games.forEach(game => {
        let predicted = predictionHistory.find(p => p.period === game.period);
        if (predicted && !sentResults.has(game.period)) { // Cek jika belum dikirim
            let result = game.amount % 10;
            let actual = result <= 4 ? "Kecil" : "Besar";
            let status = (actual === predicted.prediction) ? "✅ Menang" : "❌ Kalah";

            // Update riwayat prediksi
            predicted.result = status;

            sendPredictionHistory(); // Update Telegram
            sentResults.add(game.period); // Tandai sebagai sudah dikirim
        }
    });
}

// Fungsi mengirim riwayat prediksi ke Telegram
function sendPredictionHistory() {
    let message = `📊 *Riwayat Prediksi Wingo:*\n`;
    predictionHistory.forEach(p => {
        message += `📌 ${p.period} | 🎲 ${p.prediction} | ${p.result}\n`;
    });
...
setInterval(fetchMyEmerdList, 5000);
...
element_2.textContent = "4";
    ...
    fetch("/api/webapi/GetUserInfo")
      ...
      .then((data) => {
        ...
        $(".num span").text(`Rp ${data.data.money_user}. `);
      });
    $(".Loading").fadeOut(0);
  }, 1000);
});
...
fetch("/api/webapi/GetUserInfo")
  ...
  .then((data) => {
    ...
    $(".num span").text(`Rp ${data.data.money_user}.00 `);
  });
...
$(".foot .right").click(function (e) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $(".total-box .num span").text("Rp " + response.money + ".00");
